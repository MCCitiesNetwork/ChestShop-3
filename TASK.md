...
Modify ChestShop to use the **TreasuryApi** directly instead of the Vault Economy interface.
This enables ChestShop to support both **PERSONAL** and **BUSINESS** accounts natively — a player can configure a chest shop to withdraw from / deposit to a specific business account rather than always using their personal balance.
...
The accountId should be encoded in base 36, that's a case-insensitive combination of 0-9 A-Z on the sign to maximise the amount of accounts possible in the 15-character sign width limit (2 characters for the B: so 13 remaining)
...
Authorisors of an account should also be able to access the chestshop inventories.